...
class Admins extends CI_Controller {
    ...
    public function instructors($param1 = "", $param2 = "") {
        ...
        if($param1 == "add"){
            $this->master_model->add_user(true); // PROVIDING TRUE FOR INSTRUCTOR
            redirect(site_url('admin/instructors'), 'refresh');
        }
        elseif ($param1 == "edit") {
            $this->master_model->edit_user($param2);
            redirect(site_url('admin/instructors'), 'refresh');
        }
        elseif ($param1 == "delete") {
            $this->master_model->delete_user($param2);
            redirect(site_url('admin/instructors'), 'refresh');
        }

        $page_data['page_name'] = 'instructors';
        $page_data['page_title'] = get_phrase('instructor');
        $page_data['instructors'] = $this->master_model->get_instructor()->result_array();
        $this->load->view('backend/index', $page_data);
    }
    ...
    public function users($param1 = "", $param2 = "") {
        ...
        $page_data['users'] = $this->master_model->get_user($param2);
        $this->load->view('backend/index', $page_data);
    }
    ...
    public function get_courses() {
        ...
        if(!empty($courses)) {
            foreach ($courses as $key => $row) {
                $instructor_details = $this->master_model->get_all_user($row->user_id)->row_array();
    ...
    public function stripe_checkout_for_instructor_revenue($payout_id) {
        ...
        if ($payout_details['amount'] > 0 && $payout_details['status'] == 0) {
            $page_data['user_details']    = $this->master_model->get_user($payout_details['user_id'])->row_array();
    ...
    public function instructor_application($param1 = "", $param2 = ""){ // param1 is the status and param2 is the application id
        ...
        if ($param1 == 'approve' || $param1 == 'delete') {
            $this->master_model->update_status_of_application($param1, $param2);
        }
        $page_data['page_name']  = 'application_list';
        $page_data['page_title'] = get_phrase('instructor_application');
        $page_data['approved_applications'] = $this->master_model->get_approved_applications();
        $page_data['pending_applications'] = $this->master_model->get_pending_applications();
        $this->load->view('backend/index', $page_data);
    }
