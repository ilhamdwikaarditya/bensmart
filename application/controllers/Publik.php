...
class Publik extends CI_Controller
{
    ...
    public function class_all()
    {
        ...
        if (isset($_GET['jenjang']) && !empty($_GET['jenjang'])) {
            $selected_jenjang = $_GET['jenjang'];
        }

        // Get the category ids
        if (isset($_GET['materi']) && !empty($_GET['materi'] && $_GET['materi'] != "all")) {
        ...
        if ($selected_jenjang == "all" && $selected_materi == "all" && $selected_materisub == "all" && $selected_price == "all" && $selected_rating == 'all') {
            ...
            $this->db->select('a.*, b.kd_jenjang, b.nm_jenjang, substring(desc_class, 1, 100) as desc_short');
            ...
            // $this->pagination->initialize($config);
            $this->db->select('a.*, substring(a.desc_class, 1, 100) as desc_short, b.kd_jenjang, b.nm_jenjang');
            $this->db->from('tr_class a');
            $this->db->join('ref_jenjang b', 'a.id_jenjang = b.id_jenjang', 'left');

            if ($selected_jenjang != "all") {
                $this->db->where("b.kd_jenjang = '$selected_jenjang'");
            }
            ...
            $this->db->where('active', '1');
            $page_data['courses'] = $this->db->get()->result_array();
            // $courses = $this->master_model->filter_class($selected_jenjang, $selected_materi, $selected_materisub, $selected_price, $selected_rating);
    ...
    public function desc_class($id_class)
    {
        if (!$this->session->userdata('layout')) {
            $this->session->set_userdata('layout', 'list');
        }
    ...
    public function courses()
    {
        ...
        $layout = $this->session->userdata('layout');
        $selected_materi_id = "all";
        $selected_price = "all";
        $selected_level = "all";
        $selected_language = "all";
        $selected_rating = "all";
        // Get the category ids
        if (isset($_GET['nm_materi_group']) && !empty($_GET['nm_materi_group'] && $_GET['nm_materi_group'] != "all")) {
            $selected_materi_id = $this->master_model->get_materi_group_id($_GET['nm_materi_group']);
        }
        ...
        if ($selected_materi_id == "all" && $selected_price == "all" && $selected_level == 'all' && $selected_language == 'all' && $selected_rating == 'all') {
            $this->db->where('active', '1');
            $total_rows = $this->db->get('tr_class')->num_rows();
            $config = array();
            $config = pagintaion($total_rows, 6);
            ...
            $courses = $this->master_model->filter_class($selected_materi_id, $selected_price, $selected_level, $selected_language, $selected_rating);
            $page_data['courses'] = $courses;
        }

        $page_data['page_name']  = "courses_page";
        $page_data['page_title'] = 'Daftar Kelas';
        $page_data['layout']     = $layout;
        $page_data['selected_materi_id']     = $selected_materi_id;
